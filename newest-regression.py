...
## read in the dataset
#
df = pd.read_csv("FinalDataset.csv") # TODO; current stim

layers = [-4, -3, -2, -1]
eng_model = AutoModel.from_pretrained(model, output_hidden_states=True)  # define the model as the pre-trained BERT model
pt_model = AutoModel.from_pretrained(model, output_hidden_states=True)  # define the model as the pre-trained BERT model
eng_tokenizer = AutoTokenizer.from_pretrained(model)  # define the tokenizer as BERT's pre-trained tokenizer
pt_tokenizer = AutoTokenizer.from_pretrained(model)  # define the tokenizer as BERT's pre-trained tokenizer

eng_space = torch.zeros((768, len(df)))
pt_space = torch.zeros((768, len(df)))
for inx in range(eng_space.shape[1]):
    eng_space[:, inx] = get_word_vector(df.english_colloc[inx], eng_tokenizer, eng_model, layers)
    pt_space[:, inx] = get_word_vector(df.port_colloc[inx], pt_tokenizer, pt_model, layers)

eng_space = normalize(eng_space, ['unit', 'center', 'unit'])
pt_space = normalize(eng_space, ['unit', 'center', 'unit'])

## check to see if it loaded correctly, etc.
# print(df)
...
class CollocDataset(Dataset):
    ...
    ## it also takes  the following arguments when instantiated: pairs, model, context, layers
    #
    ...
    def __getitem__(self, inx):
        return eng_space[:, inx], pt_space[:, inx]
...
class CollocNet(nn.Module):
    ...
    def forward(self, x):  # forward is what an nn.Module uses when called
        translated_eng = self.stack(x)
        return translated_eng  # returns output in logits


## so now let's set up a function to train our model
#
def train(dataloader, model, loss_fn, optimizer):
    ...
    for batch, (X, y) in enumerate(dataloader):
        X, y = X.to(device), y.to(
            device)  # convert training and test tensors to GPU compatible versions if the machine has a GPU (?)
...
for k in range(folds):
    ...
    #
    training_data = CollocDataset((df[(df['k'] != k + 1)]), 'distilbert-base-uncased') #TODO: needs updating with pt and eng bert
    test_data = CollocDataset((df[(df['k'] == k + 1)]), 'distilbert-base-uncased') #TODO: needs updating with pt and eng bert

    train_dataloader = DataLoader(training_data, batch_size=16,
                                  shuffle=True)  # batch size of 16 converges faster than sizes 1 and 32
    ...
    #
    torch.save(model.state_dict(), f"colloc_model_fold_{k + 1}_epoch{t + 1}.pth")
